...
class Repeater(Container):
    ...
    ### added by mib
    def emit(self, fn= None, *args):
        """ Render one instance of this node."""
        self.__renderedContent = collector = []
        clone = self._fastClone()
        if fn:
            fn(clone, *args)
            if not clone._omit:
...
class RichContent(Content):
    ...
    def __init__(self, content, encoder, decoder):
        ...
        ### mib: added
        # python 2.0 way for content[1::2]
        tmp = [content[x] for x in range(1,len(content),2)]
        ###
        ### mib: changed
        #self.__nodesDict = dict(
        #        [(node._nodeName, node) for node in content[1::2]]) # (On clon-
        self.__nodesDict = dict(
                [(node._nodeName, node) for node in tmp]) # (On clon-
        ###
...
class HTMLTemplate(RichContent):
